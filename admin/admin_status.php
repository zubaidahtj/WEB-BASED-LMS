...
<body>
    ...
    {
        $q=mysqli_query($db,"SELECT first, last, username, phone_number, email FROM `admin`  WHERE username LIKE '%$_POST[search]%' 
        AND status=''");
    ...
    /*if button is not pressed*/
        ...
        $res=mysqli_query($db, "SELECT first, last, username, phone_number, email FROM `admin` WHERE status='';");
        echo "<table class='table table-bordered table-hover'>";
        echo "<tr style= 'background-color: #f1dcdc;'>";
        //table header of member table
        echo "<th>"; echo "First Name"; echo "</th>";
